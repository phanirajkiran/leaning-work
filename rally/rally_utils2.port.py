...
def list_feature(rally,feature):
    release = "Linux OVP 8.0"
    userstory = "US60061"
    #response = rally.get(feature,query='FormattedID = %s' % userstory)
    #response = rally.get(feature,query='FormattedID = "F4561"')
    response = rally.get(feature,query='Release.Name = "Linux OVP 7"',fetch="Name,FormattedID,UserStories")
	...
	for userstory in feature.UserStories:
	    print("    %-6s:%-49s" % (userstory.FormattedID,userstory.Name))
